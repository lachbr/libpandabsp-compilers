...
/*int TextureSize(const texref_t *tex)
{
	int size = 0;
	int w, h;
	size += sizeof(texref_t);
...
void WriteEmptyTexture(FILE *outwad, const texref_t *tex)
{
	texref_t outtex;
	int start, end;
	int w, h;
	memcpy (&outtex, tex, sizeof(texref_t));
	start = ftell (outwad);
	fseek (outwad, sizeof(texref_t), SEEK_CUR);
	w = tex->width, h = tex->height;
	for (int imip = 0; imip < MIPLEVELS; ++imip, w/=2, h/=2)
	{
	...
	SafeWrite (outwad, &outtex, sizeof (texref_t));
...
static void		WriteTextures(const char* const name)
{
	...
	if (!g_textureparse)
	{
		int dataofs = (int)(intptr_t)&((dtexlump_t*)NULL)->dataofs[((dtexlump_t*)g_dtexrefs)->numtexref];
		...
		header.numlumps = ((dtexlump_t*)g_dtexrefs)->numtexref;
		header.infotableofs = g_numtexrefs - dataofs + wadofs;
		SafeWrite (wadfile, &header, wadofs);

		SafeWrite (wadfile, (byte *)g_dtexrefs + dataofs, g_numtexrefs - dataofs);

		lumpinfo_t *info;
		info = (lumpinfo_t *)malloc (((dtexlump_t*)g_dtexrefs)->numtexref * sizeof (lumpinfo_t));
		hlassume (info != NULL, assume_NoMemory);
		memset (info, 0, header.numlumps * sizeof(lumpinfo_t));

		for (int i = 0; i < header.numlumps; i++)
		{
			int ofs = ((dtexlump_t*)g_dtexrefs)->dataofs[i];
			int size = 0;
			if (ofs >= 0)
			{
				size = g_numtexrefs - ofs;
				for (int j = 0; j < ((dtexlump_t*)g_dtexrefs)->numtexref; ++j)
					if (ofs < ((dtexlump_t*)g_dtexrefs)->dataofs[j] &&
						ofs + size > ((dtexlump_t*)g_dtexrefs)->dataofs[j])
						size = ((dtexlump_t*)g_dtexrefs)->dataofs[j] - ofs;
			}
			info[i].filepos = ofs - dataofs + wadofs;
			info[i].disksize = size;
			info[i].size = size;
			info[i].type = (ofs >= 0 && ((texref_t*)(g_dtexrefs+ofs))->offsets[0] > 0)? 67: 0; // prevent invalid texture from being processed by Wally
			info[i].compression = 0;
			strcpy (info[i].name, ofs >= 0? ((texref_t*)(g_dtexrefs+ofs))->name: "\rTEXTUREMISSING");
		}
	...
	else
	{
		...
		lumpinfo_t *info;
		info = (lumpinfo_t *)malloc (((dtexlump_t*)g_dtexrefs)->numtexref * sizeof (lumpinfo_t)); // might be more than needed
		hlassume (info != NULL, assume_NoMemory);

		fprintf (texfile, "%d\r\n", ((dtexlump_t*)g_dtexrefs)->numtexref);
		fseek (wadfile, sizeof(wadinfo_t), SEEK_SET);

		for (int itex = 0; itex < ((dtexlump_t*)g_dtexrefs)->numtexref; ++itex)
		{
			int ofs = ((dtexlump_t*)g_dtexrefs)->dataofs[itex];
			texref_t *tex = (texref_t*)(g_dtexrefs+ofs);
			if (ofs < 0)
			{
				fprintf (texfile, "[-1]\r\n");
			}
...
static void		ReadTextures(const char *name)
{
	...
	if (!g_textureparse)
	{
		...
		((dtexlump_t*)g_dtexrefs)->numtexref = header.numlumps;
		int dataofs = (int)(intptr_t)&((dtexlump_t*)NULL)->dataofs[((dtexlump_t*)g_dtexrefs)->numtexref];
		g_numtexrefs = header.infotableofs - wadofs + dataofs;

		SafeRead (wadfile, (byte *)g_dtexrefs + dataofs, g_numtexrefs - dataofs);
		...
		for (int i = 0; i < header.numlumps; i++)
		{
			((dtexlump_t*)g_dtexrefs)->dataofs[i] = info[i].filepos - wadofs + dataofs;
		}
	...
	else
	{
		...
		int numtexref = 0;
		if (skipspace (texfile), fscanf (texfile, "%d", &numtexref) != 1)
			Error ("File read failure");
		((dtexlump_t*)g_dtexrefs)->numtexref = numtexref;
		g_numtexrefs = (byte *)(&((dtexlump_t*)g_dtexrefs)->dataofs[numtexref]) - g_dtexrefs;

		for (int itex = 0; itex < numtexref; ++itex)
		{
			int len;
			...
			if (len < 0)
			{
				((dtexlump_t*)g_dtexrefs)->dataofs[itex] = -1;
			}
			else
			{
				char name[128];
				...
				((dtexlump_t*)g_dtexrefs)->dataofs[itex] = g_numtexrefs;
				texref_t *tex = (texref_t*)(g_dtexrefs + g_numtexrefs);
				int j;
				...
				if (j == header.numlumps)
				{
					...
					if (skipspace (texfile), fscanf (texfile, "%d", &h) != 1)
						Error ("File read failure");
					g_numtexrefs += sizeof(texref_t);
					hlassume(g_numtexrefs < g_max_map_texref, assume_MAX_MAP_MIPTEX);
					memset (tex, 0, sizeof(texref_t));
