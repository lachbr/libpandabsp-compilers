...
bool            CheckFaceForNull(const face_t* const f)
{
    ...
    if (g_bUseNullTex)
    {
        ...
        ofs = ((dtexlump_t*)g_dtexrefs)->dataofs[info->texref];
        texref = (texref_t*)(&g_dtexrefs[ofs]);

        if (!strcasecmp(texref->name, "null"))
            return true;
	#ifdef HLCSG_CUSTOMHULL
        else if (!strncasecmp(texref->name, "null", 4))
            return true;
	#else
        else
            return false;
...
bool            CheckFaceForEnv_Sky(const face_t* const f)
{
	...
	return false;
        ...
        if (!strcasecmp(texref->name, "env_sky"))
            return true;
        else
            return false;
#endif
...
bool            CheckFaceForHint(const face_t* const f)
{
    ...
    if (!strcasecmp(texref->name, "hint"))
    {
        return true;
    }
    else
...
bool            CheckFaceForSkip(const face_t* const f)
{
    ...
    if (!strcasecmp(texref->name, "skip"))
    {
        return true;
    }
    else
