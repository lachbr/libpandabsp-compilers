...
// =====================================================================================
int	ParseImplicitTexinfoFromTexture (int texref)
{
	int texinfo;
	int numtextures = g_numtexrefs;
	int size;
	...
	{
		Warning ("ParseImplicitTexinfoFromTexture: internal error: invalid texture number %d.", texref);
		return -1;
	}
...
int ParseTexinfoForFace (const dface_t *f)
{
	...
	texinfo = f->texinfo;
	texref = g_texinfo[texinfo].texref;
	if (texref != -1)
	{
		texinfo2 = ParseImplicitTexinfoFromTexture (texref);
		if (texinfo2 != -1)
		{
			texinfo = texinfo2;
		}
...
void DeleteEmbeddedLightmaps ()
{
	...
	// Step 2: remove redundant texinfo
	{
		...
		for (i = g_numtexinfo - 1; i > -1; i--)
		{
			...
			if (info->texref < 0 || info->texref >= numtextures)
			{
				break; // invalid; should not remove this texinfo
			}
			if (ParseImplicitTexinfoFromTexture (info->texref) == -1)
			{
